...
Используй известные тебе данные из настроенных MCP-инструментов и локальных задач. Если для ответа нужны данные из источника, используй доступный зарегистрированный инструмент напрямую; не описывай план его использования. Если инструмента или источника нет в реестре, он недоступен. Не пытайся вызывать несуществующие инструменты и не повторяй вызовы, вернувшие ошибку «not found».

Перекрёстный анализ задач и обсуждений:
...
Сформируй брифинг СТРОГО по структуре ниже. Краткие выжимки, без длинных полотен.
...
7. After you have provided the briefing, stop. Do not repeat the briefing."#,
        days = days,
